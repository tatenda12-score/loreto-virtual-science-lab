...
class Submission(Base):
    ...
    The ``automatic_score`` is populated automatically by the grading
    service that compares observations against Experiment.parameters.
    The ``final_score`` is either the automatic score or a teacher override.
    """
    ...
    # ── Grading ───────────────────────────────────────────────────────
    automatic_score: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Auto-calculated score (0-100) generated by the system",
    )
    final_score: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Final score (0-100), either automatic or teacher-overridden",
    )
    graded_by_id: Mapped[Optional[int]] = mapped_column(
        Integer,
        ForeignKey("users.id", ondelete="SET NULL", name="fk_submission_graded_by"),
        nullable=True,
        comment="Teacher who finalized the grade (if applicable)",
    )
    graded_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        comment="Timestamp when the teacher finalized the grade",
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint("automatic_score >= 0 AND automatic_score <= 100", name="chk_automatic_score_range"),
        CheckConstraint("final_score >= 0 AND final_score <= 100", name="chk_final_score_range"),
    )
